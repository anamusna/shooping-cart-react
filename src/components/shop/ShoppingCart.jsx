...
class ShoppingCart extends React.Component {
	...
	calculateSummary = () => {
		...
		Object.keys(this.props.items).map((id) => {
			...
			totalPrice =
				totalPrice + this.calculatePrice(this.props.items[id].product.price, this.props.items[id].amount);
	...
	calculatePrice = (price, amount) => {
		...
		if (price > 0) {
			price = Math.round(price * this.state.currencyRate * amount * 100) / 100;
		}
	...
	changeCurrency = (obj) => {
		...
		if (!this.isValidValue(obj, 'name', 'string') || !this.isValidValue(obj, 'rate', 'number')) {
			return;
		}
	...
	render() {
		return (
			<div className="container" width="200" style={{ backgroundColor: '#ccc', padding: '20px' }}>
				<div style={{ float: 'right' }}>
					<CurrencyChanger onCurrencyChanged={this.changeCurrency} current={this.state.currency} />
				</div>
				Shopping cart
				<hr />
				<table>
